...
const CATEGORY_COLORS: Record<string, string> = {
  'Rekap Minites':                    'bg-teal-500/20 text-teal-300 border-teal-500/30',
}
...
const LS_KEY = 'quiz_biasa_progress_minitesmetopen'

interface SavedBiasaState {
  selectedCategory: string
  shuffleOn: boolean
  activeQuestions: Question[]
  answersById: Record<string, AnswerState>  // key = question id
}
...
export default function Quiz() {
  ...
  // Jawaban disimpan per id soal — tetap ada saat ganti topik
  const [biasaAnswersById, setBiasaAnswersById]   = useState<Record<string, AnswerState>>({})
  ...
  const [isMuted, setIsMuted] = useState(() => localStorage.getItem('quiz_muted_ot2') === 'true')
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('quiz_muted_ot2', String(isMuted))
  }, [isMuted])
  ...
  useEffect(() => {
    ...
    if (saved && saved.activeQuestions.length > 0) {
      ...
      setBiasaAnswersById(saved.answersById ?? {})
      setBiasaActive(true)
    } else {
  ...
  // ── Derived answers for active questions (by id) ──
  const biasaAnswers = biasaQuestions.map(q => biasaAnswersById[q.id] ?? null)
  ...
  useEffect(() => {
    ...
    saveBiasa({
      ...
      answersById: biasaAnswersById,
    })
  }, [biasaActive, biasaCategory, biasaShuffleOn, biasaQuestions, biasaAnswersById])
  ...
  const handleBiasaFilterChange = (cat: string) => {
    ...
    if (!qs.length) return
    setBiasaQuestions(qs)
    // Jawaban tidak perlu diubah — tetap terbaca dari biasaAnswersById berdasarkan id soal
  }

  const handleBiasaShuffleToggle = () => {
    const nextShuffle = !biasaShuffleOn
    setBiasaShuffleOn(nextShuffle)
    const qs = buildQuestions(biasaCategory, nextShuffle, questions)
    if (!qs.length) return
    setBiasaQuestions(qs)
    // Hanya mengacak urutan — jawaban tidak direset
  }
  ...
  const handleAnswerBiasa = useCallback((qIdx: number, optIdx: number) => {
    ...
    const qId = biasaQuestions[qIdx]?.id
    if (!qId) return
    setBiasaAnswersById(prev => {
